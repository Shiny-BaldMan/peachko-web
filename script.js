...
async function loadArtists() {
  ...
  snapshot.forEach((doc) => {
    ...
    const id = doc.id; // 파이어베이스가 부여한 고유 ID

    // 클릭하면 artist.html로 가는데, 뒤에 ?id=고유값 을 붙여서 보냅니다.
    const card = `
      <div class="service-card" onclick="location.href='artist.html?id=${id}'">
        <div class="svc-img-wrap"><img src="${artist.photoUrl}" class="svc-img"></div>
        <h3 class="fd">${artist.name}</h3>
        <p class="fb">${artist.intro}</p>
      </div>
    `;
...
// 마지막으로 init 부분에 loadArtists(); 를 추가하세요.
